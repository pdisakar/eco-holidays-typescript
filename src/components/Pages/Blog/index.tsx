...
export default function Blog({ data, siteUrl }: BlogProps) {
  ...
  useEffect(() => {
    const blockEls =
      document.querySelectorAll<HTMLDivElement>('div.package-block');

    for (let i = 0; i < data.block_count; i++) {
      const blocks = `data.block${i + 1}`;
      if (!blockEls[i]) continue;

      blockEls[i].removeAttribute('style');
      blockEls[i].innerHTML = eval(blocks).map((itm: PackageItem) => {
        const {
          package_title,
          featured,
          package_discount,
        ...
        const featured_image = featured
          ? `<a href="${PACKAGE_BASE_URL}${slug}" class="image-slot">
        ...
        const tripStatus = additional_field_1
          ? additional_field_1 === 'Best Price'
            ? `<div class="status danger"><span class="icon">${svgTagsFill}</span>${additional_field_1}</div>`
            : additional_field_1 === 'Group Tours'
            ? `<div class="status success"><span class="icon">${svgPeopleFill}</span>${additional_field_1}</div>`
            : `<div class="status warning"><span class="icon">${svgStarFill}</span>${
          ...
          <div class="item">
            ...
            <figcaption>
              <div class="caption-header">
                <h3><a href="${PACKAGE_BASE_URL}${slug}">${package_title}</a></h3>
              </div>
              <p><span class="duration">${packageDuration}</span> from <span class="discount">US $${(
          group_default_price +
          group_default_price * 0.075
        ).toFixed(0)}</span>
        ...
        <div className="content container">
          <div className="lg:w-2/3 lg:mx-auto">
            ...
            <EnquireUs
              ...
              type="Contact Us"
              classes="flm-form mb-10 bg-white shadow-base p-6 lg:p-8 rounded"
            />
          </div>
        </div>
      </div>
